@extends('layout.main')
@section('title', 'Record Reagent Usage')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-xl font-semibold text-gray-800">Record Usage</h2>
            <p class="text-sm text-gray-500 mt-1">Log reagent consumption and update inventory.</p>
        </div>
        <div class="flex flex-wrap gap-3">
            @can('admin')
            <a href="{{ route('take-admin', ['guid' => $reagen->guid]) }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                Admin Mode
            </a>
            @endcan
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <form action="{{ route('take.process', ['noCatalog' => $reagen->noCatalog]) }}" method="POST" class="p-6 space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label for="batchSelect" class="block text-sm font-medium text-gray-700 mb-1.5">Batch Number</label>
                    ...
                    <div>
                        ...
                        <input type="number" id="quantity_taken" name="quantity_taken" value="{{ old('quantity_taken') }}" required min="1" max="{{ $reagen->stockReagen?->quantity }}" placeholder="0"
...
@if($errors->has('quantity_taken') || session('error'))
@push('scripts')
