...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Track and reconcile reagent inventory for {{ \Carbon\Carbon::createFromDate($year ?? date('Y'), $month ?? date('m'), 1)->format('F Y') }}.</p>
        </div>
        <button type="button" class="inline-flex items-center px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors shadow-sm">
    ...
    <div>
        <h3 class="text-sm font-semibold text-gray-700 mb-3">Select Period</h3>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            @php
                // Fallback structure jika controller belum mengirim $availableMonths
                $periods = $availableMonths ?? [];
            @endphp
            @forelse($periods as $p)
                @php
                    $date = \Carbon\Carbon::createFromDate($p['year'], $p['month'], 1);
                    $isActive = ($p['month'] == ($month ?? date('m')) && $p['year'] == ($year ?? date('Y')));
                    $statusClass = $p['status'] === 'completed' 
                        ? 'border-emerald-200 bg-emerald-50/50' 
                        : 'border-gray-200 bg-white hover:border-blue-300 hover:shadow-sm';
                @endphp
                <a href="?year={{ $p['year'] }}&month={{ $p['month'] }}"
                   class="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all {{ $isActive ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500/20' : $statusClass }}">
                    <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $date->format('M') }}</span>
                    <span class="text-lg font-bold text-gray-900">{{ $date->format('Y') }}</span>
                    <span class="mt-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full {{ $p['status'] === 'completed' ? 'text-emerald-700 bg-emerald-100' : 'text-gray-400 bg-gray-100' }}">
            ...
            @empty
                <div class="col-span-full text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                    <p class="text-sm text-gray-500">No periods available. Generate stock to start.</p>
                </div>
            @endforelse
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-100">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">SO Status</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-center text-gray-900" id="qty_now_{{ $reagen->id }}">
                            {{ $reagen->stock_opname ? $reagen->quantity_actual : $reagen->quantity }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="text-xs text-gray-500">{{ $reagen->status ?? '-' }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full {{ $reagen->stock_opname ? 'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20' : 'bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20' }}">
                                {{ $reagen->stock_opname ? 'Done' : 'Pending' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <button type="button" class="edit-btn inline-flex items-center px-2.5 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors"
                                    ...
                                    data-current="{{ $reagen->stock_opname ? $reagen->quantity_actual : $reagen->quantity }}"
